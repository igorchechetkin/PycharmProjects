...
# Человек хочет сделать рассылку поздравлений для определённого списка людей. Поздравления для разных людей
# он хочет написать по-разному.
#
...
print('Введите шаблон поздравления, в шаблоне нужно использовать конструкцию {name} и {age}:')
pattern_congratulations = input()

peoples = input('Список людей через запятую: ').split(', ')

ages = input('Возраст людей через пробел: ').split()

for i_man in range(len(peoples)):
    print('\n', pattern_congratulations.format(name=peoples[i_man], age=ages[i_man]))

new_peoples_list = [(' '.join([peoples[i_man], ages[i_man]])) for i_man in range(len(peoples))]

print('\nИменинники:', ', '.join(new_peoples_list))
