# Задача 2. Семья
# Реализуйте класс «Семья», который состоит из атрибутов «Имя», Деньги» и «Наличие дома» и объекты
# которого могут выполнять следующие действия:
#
# Отобразить информацию о себе.
# Заработать денег (подаётся число, которое прибавляется к текущему значению денег).
# Купить дом (подаётся стоимость дома и необязательный аргумент «Скидка»). Вывести соответствующее
# сообщение об успешной/неуспешной покупке дома.
...
class Family:
    ...
    def print_info(self):
        print(f"Family name: {self.surname}\nFamily funds: {self.money}\nHaving a house: {self.have_a_house}")

    def earn_money(self, amount):
        self.money += amount
        print(f"\nEarned {amount} money! Current value: {self.money}\nTry to buy a house again")
...
family_1.buy_house(10 ** 6)
family_1.earn_money(800000)
family_1.buy_house(10 ** 6, 20)
family_1.print_info()
